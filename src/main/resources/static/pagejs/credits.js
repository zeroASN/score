        ...
        $.ajax({
            ...
            if(result.code>=0){
                ...
                $.each(data, function (key, value) {
                    ...
                    const $field = $("#creditsForm [name='" + key + "']");
...
layui.use(function () {
    ...
    table.render({
        elem: '#tbCredits',
        url: '/api/credits/getbypage', // 此处为静态模拟数据，实际使用时需换成真实接口
        ...
        cols: [[
            ...
            {field: 'professional_core', title: '专业核心课', sort: true},
            {field: 'professional_development', title: '专业拓展课', sort: true},
            {field: 'general_education', title: '通识选修课', sort: true},
    ...
    // 触发单元格工具事件
    table.on('tool(tbCredits)', function (obj){

        var data = obj.data; // 获得当前行数据
        // console.log(obj)
        if(obj.event === 'edit'){
            // 调用已有的 showStudenDlg 函数，传入学生ID
            showCreditsDlg(data.id);
        }
    });
...
function search(){
    let credits = getSearchCondtion();

    const table = layui.table;
    table.reloadData("tbCredits", {
        where: {data:credits}
    });
    console.log("where condition:"+JSON.stringify(credits))
}

function deleteConfirm() {
    const table = layui.table;
    // 获取表格的选中状态
    const checkStatus = table.checkStatus('tbCredits'); // 'tbStudent' 是你的表格的 ID 或 lay-filter
    ...
    layer.confirm('你真的要删除这些学生吗？一旦删除，不可恢复！', {icon: 3}, function () {
        const deletePromises = data.map(item => {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: "/api/credits/delete/" + item.id,
                    method: "DELETE"
                }).done(result => {
                    resolve(result);
...
function commitCreditsDlg() {
    ...
    return new Promise((resolve, reject)=> {
        ...
        const id = $("#id").val();
        const formData = $(".layui-layer-page #creditsForm").serialize();
        const url = id ? "/api/credits/update" : "/api/credits/add";
